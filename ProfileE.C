...
void ProfileE(){
   ...
   gROOT->ProcessLine("gErrorIgnoreLevel = kFatal;");
    ...
    // Open the root files
    TFile *fileEposLHC = new TFile("GAMMA_EPOSLHC_7TEV_10M_ON.root");
    TFile *fileEpos4 = new TFile("GAMMA-EPOS4_7TEV_1M_mimic.root");
    TFile *filePythia = new TFile("GAMMAPYTHIA_MB_ON_CR.root");
 ...
 //MC simulations----------------------------------------------------------------
    ...
    TProfile *profilek1 = (TProfile*)fileEposLHC->Get("Gammak_pfx");
    TProfile *profilek2 = (TProfile*)fileEpos4->Get("Gammak_pfx");
    TProfile *profilek3 = (TProfile*)filePythia->Get("Gammak_pfx");
    
    TProfile *profilel1 = (TProfile*)fileEposLHC->Get("Gammal_pfx");
    TProfile *profilel2 = (TProfile*)fileEpos4->Get("Gammal_pfx");
    TProfile *profilel3 = (TProfile*)filePythia->Get("Gammal_pfx");
    
    TProfile *profilex1 = (TProfile*)fileEposLHC->Get("Gammax_pfx");
    TProfile *profilex2 = (TProfile*)fileEpos4->Get("Gammax_pfx");
    TProfile *profilex3 = (TProfile*)filePythia->Get("Gammax_pfx");
    
    TProfile *profileo1 = (TProfile*)fileEposLHC->Get("Gammao_pfx");
    TProfile *profileo2 = (TProfile*)fileEpos4->Get("Gammao_pfx");
    TProfile *profileo3 = (TProfile*)filePythia->Get("Gammao_pfx");
 ...
 //ALICE DATA----------------------------------------------------------------
    ...
    // Extract graphs from HEP1.root, HEP2.root, ..., HEP5.root
    ...
    for (int i = 0; i < Datak->GetN(); ++i) {
    std::cout << "Point " << i << ": X = " << Datak->GetX()[i]
              << ", Y = " << Datak->GetY()[i]
              << ", ErrorX = " << Datak->GetErrorX(i)
              << ", ErrorY = " << Datak->GetErrorY(i) << std::endl;
}
...
//----------------------------------------------------------------------------
    ...
    TCanvas *canvas = new TCanvas("canvas", "Histograms and Profiles", 600, 1100);
  ...
  // Create a new pad with specific ranges
   ...
   Dataproton->SetLineColor(kViolet-1); 
...
//TGRAPH------------------------------------------------------------- 
    ...
    Datak->Draw("PE same"); // "P" option for drawing markers
    Datalambda->Draw("PE same");
    Datalambda->Draw("PE same");	
    Dataxi->Draw("PE same");
    Dataomega->Draw("PE same");
...
// Draw the TProfiles-----------------------------------------
  ...
  //  profilep3->Draw("same");
    ...
    profilek2->SetMarkerStyle(10);
    profilek2->SetMarkerColor(kBlack);
    profilek2->SetLineColorAlpha(kGreen,0);
    profilek2->Draw("same");
    ...
    profilel2->SetMarkerStyle(10);
    profilel2->SetMarkerColor(kBlue);
    profilel2->SetLineColorAlpha(kGreen,0);
    profilel2->Draw("same");
    ...
    profilex2->SetMarkerStyle(10);
    profilex2->SetMarkerColor(kGreen);
    profilex2->SetLineColorAlpha(kGreen,0);
    profilex2->Draw("same");
   ...
   profileo1->Draw("same");
    ...
    profileo2->SetMarkerStyle(10);
    profileo2->SetMarkerColor(kRed);
    profileo2->SetLineColorAlpha(kGreen,0);
    profileo2->Draw("same");
...
// Convert TProfile to TGraphErrors to have control over the drawing style
const int nk2 = profilek2->GetNbinsX(); // Number of points in the TProfile
double *xk2 = new double[nk2];
double *yk2 = new double[nk2];
double *exk2 = new double[nk2]; // Errors in x (set to 0 if not needed)
double *eyk2 = new double[nk2]; // Errors in y

for (int i = 0; i < nk2; ++i) {
    // Assuming the x value is the center of the bin
    xk2[i] = profilek2->GetBinCenter(i+1);
    yk2[i] = profilek2->GetBinContent(i+1);
    exk2[i] = 0; // No error in x
    eyk2[i] = profilek2->GetBinError(i+1); // Error in y
}

// Create TGraphErrors with the data from the TProfile
TGraphErrors *grOk2 = new TGraphErrors(nk2, xk2, yk2, exk2, eyk2);
grOk2->SetFillColorAlpha(kBlack,0.3); // Set fill color to red
grOk2->SetLineColorAlpha(kBlue,0); // Set line color to red
grOk2->SetFillStyle(1001); // Solid fill style

// Draw the graph with error bars and a smooth colored surface
grOk2->Draw("3"); // "3" option to draw a smooth surface
...
// Convert TProfile to TGraphErrors to have control over the drawing style
const int nl2 = profilel2->GetNbinsX(); // Number of points in the TProfile
double *xl2 = new double[nl2];
double *yl2 = new double[nl2];
double *exl2 = new double[nl2]; // Errors in x (set to 0 if not needed)
double *eyl2 = new double[nl2]; // Errors in y

for (int i = 0; i < nl2; ++i) {
    // Assuming the x value is the center of the bin
    xl2[i] = profilel2->GetBinCenter(i+1);
    yl2[i] = profilel2->GetBinContent(i+1);
    exl2[i] = 0; // No error in x
    eyl2[i] = profilel2->GetBinError(i+1); // Error in y
}

// Create TGraphErrors with the data from the TProfile
TGraphErrors *grOl2 = new TGraphErrors(nl2, xl2, yl2, exl2, eyl2);
grOl2->SetFillColorAlpha(kBlue,0.3); // Set fill color to red
grOl2->SetLineColorAlpha(kBlue,0); // Set line color to red
grOl2->SetFillStyle(1001); // Solid fill style

// Draw the graph with error bars and a smooth colored surface
grOl2->Draw("3"); // "3" option to draw a smooth surface
...
// Convert TProfile to TGraphErrors to have control over the drawing style
const int nx2 = profilex2->GetNbinsX(); // Number of points in the TProfile
double *xx2 = new double[nx2];
double *yx2 = new double[nx2];
double *exx2 = new double[nx2]; // Errors in x (set to 0 if not needed)
double *eyx2 = new double[nx2]; // Errors in y

for (int i = 0; i < nx2; ++i) {
    // Assuming the x value is the center of the bin
    xx2[i] = profilex2->GetBinCenter(i+1);
    yx2[i] = profilex2->GetBinContent(i+1);
    exx2[i] = 0; // No error in x
    eyx2[i] = profilex2->GetBinError(i+1); // Error in y
}

// Create TGraphErrors with the data from the TProfile
TGraphErrors *grOx2 = new TGraphErrors(nx2, xx2, yx2, exx2, eyx2);
grOx2->SetFillColorAlpha(kGreen,0.3); // Set fill color to red
grOx2->SetLineColorAlpha(kGreen,0); // Set line color to red
grOx2->SetFillStyle(1001); // Solid fill style

// Draw the graph with error bars and a smooth colored surface
grOx2->Draw("3"); // "2" option to draw a smooth surface
...
// Convert TProfile to TGraphErrors to have control over the drawing style
const int n2 = profileo2->GetNbinsX(); // Number of points in the TProfile
double *x2 = new double[n];
double *y2 = new double[n];
double *ex2 = new double[n]; // Errors in x (set to 0 if not needed)
double *ey2 = new double[n]; // Errors in y

for (int i = 0; i < n2; ++i) {
    // Assuming the x value is the center of the bin
    x2[i] = profileo2->GetBinCenter(i+1);
    y2[i] = profileo2->GetBinContent(i+1);
    ex2[i] = 0; // No error in x
    ey2[i] = profileo2->GetBinError(i+1); // Error in y
}

// Create TGraphErrors with the data from the TProfile
TGraphErrors *grO2 = new TGraphErrors(n2, x2, y2, ex2, ey2);
grO2->SetFillColorAlpha(kRed,0.3); // Set fill color to red
grO2->SetLineColorAlpha(kRed,0); // Set line color to red
grO2->SetFillStyle(1001); // Solid fill style

// Draw the graph with error bars and a smooth colored surface
grO2->Draw("3"); // "3" option to draw a smooth surface
...
//----------------------------------------------------------------
    ...
    TLegend* legend = new TLegend(0.65, 0.5, 0.9, 0.8); // (x1, y1, x2, y2) coordinates for the legend
   ...
   TLegend* legend1 = new TLegend(0.55, 0.25, 0.9, 0.35); // (x1, y1, x2, y2) coordinates for the legend
   //legend1->SetHeader("PYTHIA");
   legend1->SetHeader("MC");
   legend1->AddEntry(profilek1, "EPOS LHC FUSION ON", "p");
   legend1->AddEntry(profilek2, "EPOS 4 mimic HYDRO ", "p");	
   legend1->AddEntry(profilek3, "PYTHIA CR ON", "p");
